...
AYAR_DOSYASI = "ayar.json"

def _ayarlari_yukle():
    varsayilanlar = {"son_port": "", "son_kamera": None}
    if not os.path.exists(AYAR_DOSYASI):
        _ayarlari_kaydet(varsayilanlar)
...
def konveyor_portu():
    ...
    # Ayarları tek dosyadan çekiyoruz
    ayarlar = _ayarlari_yukle()
    kayitli_port = ayarlar.get("son_port", "")
    ...
    for com_num, (dev, desc, hwid) in port_sozlugu.items():
        ...
        print(f"[{com_num}] {dev} - {desc}{varsayilan_ifade}")
        print(f"    -> HWID: {hwid}")
    ...
    if secim in port_sozlugu:
        ...
        # Seçilen portu tek JSON dosyasına kaydediyoruz (kamerayı ezmeden)
        ayarlar = _ayarlari_yukle()
        ayarlar["son_port"] = secilen_device
        _ayarlari_kaydet(ayarlar)
...
def kamera_portu(max_kontrol = 5):
    ayarlar = _ayarlari_yukle()
    kayitli_index = ayarlar.get("son_kamera", None)
    ...
    if secilen_index in aktif_kameralar:
        # Seçilen kamerayı tek JSON dosyasına kaydediyoruz (portu ezmeden)
        ayarlar = _ayarlari_yukle()
        ayarlar["son_kamera"] = secilen_index
        _ayarlari_kaydet(ayarlar)
